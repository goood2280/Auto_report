...
## 자연어 규칙 (NL Rules) — 자연어로 적고, 변환 결과 확인 후 적용

> **코드/문법을 몰라도 됩니다.** 아래 `NL_RULES` 마커 사이에 규칙을 '자연어 한 줄'로 적은 뒤,
> 터미널에서 **`python Main.py --convert-nl-rules`** 를 실행하세요. 그러면:
> 1. AI(GPT OSS 120B) 연결 시 **AI가** `[RULE]`로 변환(문법 검증·1회 자동 재수정), **AI 미연결 시
>    키워드 패턴 매칭 fallback**으로 변환합니다(둘 다 되면 AI 우선).
> 2. **변환 결과(자연어→`[RULE]`)를 화면에 보여주고 `y/N` 확인**을 받습니다(발행 시 자동 적용 안 함).
> 3. 승인하면 `ANOMALY_RULES` 마커 안에 `[RULE]`을 넣고, **각 규칙 위에 `# NL: <원문>` 주석**을 남겨
>    추적 가능하게 하며, `NL_RULES`의 원문은 `<!-- (변환완료) -->` 주석으로 바꿔 재변환을 막습니다.
>
> - 확인 없이 바로 적용(배치용): `python Main.py --convert-nl-rules-yes`
> - (하위호환) `My_config.anomaly_nl_autocompile=True`로 두면 예전처럼 발행 시작 시 1회 자동 컴파일합니다.
> - **AI가 정확히 변환하려면 아래 작성 형식을 지켜주세요.**
...
> - [ ] 개수·비율 조건은 **숫자로 명시** / 조건 표현은 위 표 안의 것만 사용
> - **작성 후**: `python Main.py --convert-nl-rules` 실행 → 변환 결과를 눈으로 확인하고 `y`로 승인.
>   승인하면 아래 `ANOMALY_RULES`에 `[RULE]`이 `# NL: <원문>` 주석과 함께 추가됩니다(추적 가능).
>   표현 불가한 규칙은 `# 변환불가(키워드):` 또는 `# 변환불가:` 주석으로 남으니 수기 `[RULE]`로 보완하세요.
> - 우선순위: **수기 `[RULE]`이 먼저**, 변환된 규칙은 그 뒤. 정밀 제어(복잡한 다단계 분기)가
>   필요하면 아래 `[RULE]`을 직접 쓰는 편이 확실합니다.
> - **키워드 fallback(AI 미연결)이 인식하는 대표 표현**: `주의 이상`→`sev >= 1` · `이상`→`sev >= 2` ·
>   `둘 다 이상`(항목 2개↑)→`all_sev(…, critical)` · `산포 N배`→`disp_ratio > N` · `stddev N 이하`→`stddev < N` ·
>   `median이 spec 상한/하한의 N%`→`median >/< spec_high/low * (N/100)` · `앞부분 집중`→`seq_front_heavy` ·
>   `연속 N개 이탈`→`seq_out(N)` · `이탈 N개 이상`→`spec_out >= N` · `엣지/중앙 N%`→`zone_share(…) >= N/100`.
