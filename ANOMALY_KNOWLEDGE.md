...
## 자연어 규칙 (NL Rules) — 자연어로 적으면 발행 시 바로 적용

> **코드/문법을 몰라도 됩니다.** 아래 `NL_RULES` 마커 사이에 규칙을 '자연어 한 줄'로 적으면,
> **다음 랏 리포트 발행 때 자동으로 `when` 문법으로 변환되어 바로 적용**됩니다.
> 1. AI(GPT OSS 120B) 연결 시 **AI가** `[RULE]`로 변환(문법 검증·1회 자동 재수정), **AI 미연결 시
>    키워드 패턴 매칭 fallback**으로 변환합니다(둘 다 되면 AI 우선).
> 2. **문구별 변환 캐시** `RUN/AI/nl_rules_map.json`에 `자연어 문구 → when 코드` 매핑이 저장됩니다.
>    **같은 문구는 항상 같은 코드로 변환**(일관성)되고, 캐시에 있으면 AI를 다시 호출하지 않습니다.
> 3. **조정 방법**: ① 자연어 문구를 고쳐 다시 발행하거나, ② `nl_rules_map.json`에서 해당 문구의
>    `when`/`sev`/`note`를 직접 고치면 다음 발행부터 반영됩니다. 문구(키)를 지우면 다시 변환합니다.
>
> - **미리보기(발행 전 확인)**: `python Main.py --convert-nl-rules` — 자연어가 어떤 `when`으로
>   변환되는지 출력하고 캐시를 갱신(MD·발행 변경 없음).
> - **MD에 명시적으로 굳히기**: `python Main.py --convert-nl-rules-md` — 변환 결과를 `ANOMALY_RULES`에
>   `[RULE]`로 써 넣고(각 위에 `# NL: <원문>` 주석), `NL_RULES` 원문은 `<!-- (변환완료) -->`로 표시.
> - 발행 시 자동 적용을 끄려면 `My_config.anomaly_nl_autocompile=False`.
> - **정확히 변환되려면 아래 작성 형식을 지켜주세요.** 표현 불가한 규칙은 `# 변환불가`로 남으니 수기 `[RULE]`로.

### 작성 형식 — 한 줄에 이 4가지를 담으세요

> **`- `(하이픈+공백)로 시작하는 한 줄 = 규칙 하나.** 문장 안에:
>
> 1. **조건** — *어떤 항목이 어떤 상태일 때인지*. 항목명은 **리포트(Score Board)에 나오는 표기
>    그대로**(영문 대문자, 예: `VTH_N`, `IDSAT_P`). "VTH 쪽", "그 항목" 같은 모호한 지칭 금지.
> 2. **판정명** — 매칭 시 리포트에 표시할 불량 모드 문구. **반드시 큰따옴표**로 감싸세요: `"Gate 모듈 불량"`.
> 3. **등급(선택)** — 문장에 `(주의)`라고 쓰면 주의(WARNING), 안 쓰면 이상(CRITICAL).
> 4. **링크(선택)** — `링크: http://...` 형태로 끝에. (링크는 여기 적은 것만 리포트에 첨부됩니다.)

### 쓸 수 있는 조건 표현 — 아래 표현을 그대로 활용하세요

> | 하고 싶은 말 | 이렇게 쓰세요 |
> |---|---|
> | 항목이 spec을 벗어남 | `VTH_N이 spec 이탈이면` |
...
> | **median 값 기준(숫자)** | `VTH_N의 median 값이 10 이상이면` (이상=≥, 이하=≤, 넘으면/초과=>, 미만=<) |
> | **median이 spec 상/하한 근처** | `VTH_N이 spec 상한의 90% 넘으면` / `spec 하한의 80% 미만이면` |
> | **stddev(표준편차) 기준** | `IDSAT_N의 stddev가 0.5 이하일 때` |
> | **복합 조건** | `IDSAT_N의 산포가 2배 넘고 median이 높으면` (그리고=and) |
...
> | 두 그룹 산포 비교 | `VTH_N,VTH_P 그룹과 IDSAT_N,IDSAT_P 그룹의 산포를 비교해서 알려줘` |

### 좋은 예 ✅ / 피해야 할 예 ❌

> ✅ `- VTH_N과 VTH_P가 둘 다 spec 이탈이면 "Gate 모듈 불량"으로 판정. 링크: http://wiki/gate-cd`
> ✅ `- IDSAT_P의 median이 모집단 하위 15% 이내면 "IDSAT_P 타겟 저하"(주의)로 판정.`
> ✅ `- VTH_N이 spec 이탈일 때, 측정 순서상 연속 5개 이상 이탈하면 "측정 연속 이탈(프로브 의심)", 아니면 "VTH_N 단독 이탈"로 판정.`
> ✅ `- PCHK_LKG가 정상(주의 미만)이면 IDSAT_N의 산포 주의 언급은 하지 마.`
>
> ❌ `- VTH가 나쁘면 불량` → 항목명 불명확("VTH"가 항목인지 카테고리인지)·판정명 없음·"나쁘면"이 모호.
>    → ✅ `- VTH_N이 spec 이탈이면 "VTH_N 단독 이탈"로 판정.`
> ❌ `- 이탈이 많으면 알려줘` → "많으면"에 숫자가 없음. → ✅ `- VTH_N의 spec 이탈이 10개 이상이면 ...`
> ❌ `- 트렌드가 요즘 이상하면 판정해줘` → 지원하지 않는 조건(추세 기울기 판정 없음) → '변환불가' 처리됨.
> ❌ 판정명을 따옴표 없이: `- ... Gate 모듈 불량으로 판정` → 판정명 경계가 모호해져 오변환 위험.

### 체크리스트 & 작성 후 확인

> - [ ] 한 줄에 규칙 하나 (`- `로 시작) / 판정명은 `"큰따옴표"` / 항목명은 리포트 표기 그대로(영문 대문자)
> - [ ] 개수·비율·수치 조건은 **숫자로 명시** / 조건 표현은 위 표 안의 것만 사용
> - **작성 후**: 다음 발행 시 자동 변환·적용됩니다. 발행 전에 확인하려면 `python Main.py --convert-nl-rules`로
>   `자연어 → when` 매핑을 미리 봅니다. 터미널 `[NL RULES] 자연어 → [RULE] N개 적용`과 `[RULE CHECK]`에서 확인.
> - **일관성/조정**: 문구↔코드 매핑은 `RUN/AI/nl_rules_map.json`에 저장 — **같은 문구는 항상 같은 코드**.
>   변환이 마음에 안 들면 자연어를 고치거나, 이 파일에서 해당 문구의 `when`을 직접 수정하세요.
> - 우선순위: **수기 `[RULE]`이 먼저**, 변환된 규칙은 그 뒤. 정밀 제어(복잡한 다단계 분기)는 수기 `[RULE]` 권장.
> - **키워드 fallback(AI 미연결)이 인식하는 대표 표현**: `주의 이상`→`sev >= 1` · `이상`(등급)→`sev >= 2` ·
>   `둘 다 이상`(항목 2개↑)→`all_sev(…, critical)` · `산포/ N배`→`disp_ratio > N` · `stddev N 이상/이하`→`stddev >=/<= N` ·
>   `median 값이 N 이상/이하/넘/미만`→`median >=/<=/>/< N` · `median/값이 spec 상한/하한의 N%`→`median … spec_high/low*(N/100)` ·
>   `median 높/낮`→`median_high/low(항목)` · `앞부분 집중`→`seq_front_heavy` · `연속 N개 이탈`→`seq_out(N)` ·
>   `이탈 N개 이상`→`spec_out >= N` · `엣지/중앙 N%`→`zone_share(…) >= N/100`. 여러 조건은 `~고/그리고`로 `and` 결합.
>   (fallback은 위 명시 표현만 변환하며, 애매하면 `# 변환불가`로 남깁니다. 복잡한 규칙은 AI 변환/수기 권장.)
