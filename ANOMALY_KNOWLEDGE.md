...
> **코드/문법을 몰라도 됩니다.** 아래 마커 사이에 규칙을 '자연어 한 줄'로 적기만 하면,
> 프로그램 시작 시 AI가 `[RULE]` 포맷으로 변환하고 코드가 문법을 검증(실패 시 1회 자동 재수정)한 뒤
> 판정 규칙으로 주입합니다. 단, **AI가 정확히 변환하려면 아래 작성 형식을 지켜주세요.**
...
> - [ ] 한 줄에 규칙 하나 (`- `로 시작) / 판정명은 `"큰따옴표"` / 항목명은 리포트 표기 그대로
> - [ ] 개수·비율 조건은 **숫자로 명시** / 조건 표현은 위 표 안의 것만 사용
> - **작성 후**: 다음 발행 시 터미널 `[NL RULES] 자연어 규칙 → [RULE] N개 컴파일 완료`와
>   `[RULE CHECK]` 목록에 내 규칙이 보이는지 확인. 변환 상세(원문→[RULE]·오류)는
>   `RUN/AI/nl_rules_compiled.json`에서 확인 — 표현 불가한 규칙은 `# 변환불가:`로 남습니다.
> - 자연어 원문이 바뀌지 않으면 캐시를 재사용(LLM 재호출 없음 → 발행마다 규칙 동일).
> - 우선순위: **수기 `[RULE]`이 먼저**, 자연어 규칙은 그 뒤. 정밀 제어(복잡한 다단계 분기)가
