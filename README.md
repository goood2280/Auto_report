...
| `RUN/TEMP/anomaly_basis_<lot>.json/.csv` | Anomaly 판단 근거(device·PCHK 통합: spec-out wafer·robust 산포·이탈도·특이맵 trace + PCHK `meas_overlap_*` 동일 shot 겹침). 랏 완료 후에도 보존(이미지 파일만 정리) |
| `RUN/AI/ai_input_<lot>.md/.json` | AI에 실제 투입된 단계별 프롬프트(system/user)+응답 덤프(검증용) |
| `RUN/AI/anomaly_rule_check_<lot>_<step>.txt/.json` | **전 규칙 체크 결과**(모든 `[RULE]` 매칭/미매칭, 조건·결과·근거) |
...
└── RUN/
    ...
    ├── TEMP/anomaly_basis_<lot>.json/.csv                # Anomaly 판단 근거(보존, 이미지만 정리)
