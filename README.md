...
> ✍️ **비개발자용 작성 가이드가 `ANOMALY_KNOWLEDGE.md`의 '자연어 규칙' 섹션에 있습니다** —
> 한 줄에 담을 4요소(조건/판정명 큰따옴표/등급/링크), '쓸 수 있는 조건 표현' 표(자연어→변환 매핑),
> 좋은 예/피해야 할 예, 체크리스트. 같은 매핑 표가 컴파일 프롬프트(`NL_PATTERN_HINTS`)에도 주입되어
> **가이드대로 쓰면 변환이 사실상 결정적**으로 됩니다.
