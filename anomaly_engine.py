...
def render_findings_count_html(findings):
    """이상/주의 '건수'만 한 줄로 렌더(상세 목록 없음).

    AI 연결(지식판정) 모드에서 Anomaly Summary 아래에는 지식판정 상세 목록 대신
    '● 이상 N건 · 주의 N건' 같은 간단 요약만 표기한다(지식판정 내용은 상단 AI 해석 블록에 표시).
    """
    n_crit = sum(1 for f in (findings or []) if f.get("severity") == "CRITICAL")
    n_warn = sum(1 for f in (findings or []) if f.get("severity") == "WARNING")
    _div = ' <span style="color:#bbb;">|</span> '
    return (f'<div style="font-size:13px; color:#333; margin:4px 0;">'
            f'{_sev_dot("CRITICAL")} {_SEV_HEAD["CRITICAL"]} {n_crit}건{_div}'
            f'{_sev_dot("WARNING")} {_SEV_HEAD["WARNING"]} {n_warn}건</div>')
...
def interpret_with_ai(findings, metrics_dict, knowledge_text, llm_fn,
    ...
    try:
        ...
        _final_sys = (
            ...
            '"summary": "<종합 판단 1~2문장 — 어느 Index가 이상인지와 근거만. 개별 shot/wafer 번호 나열 금지>", '
            '"phenomenon": "<핵심 현상 1~2문장 — 어느 Index가 spec을 이탈했는지와 그 근거(측정값이 spec 이탈 / P10·MEDIAN 등 agg 집계값이 spec 이탈)만. **개별 shot·wafer·좌표·PGM(pt) 나열 금지**, 원인 추측 금지>", '
            '"actions": "<권고 조치 — 지식베이스·RULE의 NOTE에 명시된 것만. 없으면 null(임의 조치·\'미기재\' 문구 생성 금지)>", '
